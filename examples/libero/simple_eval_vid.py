...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        default=[1, 2, 3, 12, 14, 28, 29, 38],
        help="List of task IDs to evaluate (default: [1, 2, 3, 12, 14, 28, 29, 38])",
    )
    parser.add_argument(
        "--num-episodes",
        type=int,
        default=50,
        help="Number of episodes to run per task (default: 50)",
    )
    parser.add_argument(
        "--save-video",
        action="store_true",
    ...
    parser.add_argument(
        "--video-out-path",
        type=pathlib.Path,
        default=pathlib.Path("eval/pi05_libero_topk1_action/videos"),
        help="Directory to save video files (default: eval/pi05_libero_topk1_action/videos)",
    )
...
def main():
    ...
    for task_id in task_ids:
        ...
        exp_name = f"{task_suite_name}_evaltasks_trials{num_trials_per_task}"
        obs_dir = pathlib.Path("eval/pi05_libero_topk1_action/libero_final_obs") / exp_name
        traj_dir = pathlib.Path("eval/pi05_libero_topk1_action/libero_simple_trajectories") / exp_name
        ...
        for episode_idx in range(num_trials_per_task):
            ...
            # Video recording: store preprocessed frames
            ...
            if args.save_video and replay_images:
                video_path = args.video_out_path / f"task{task_id}_ep{episode_idx}_{suffix}.mp4"
                imageio.mimwrite(
                    video_path,
